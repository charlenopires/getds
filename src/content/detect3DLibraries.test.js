...
describe('detect3DLibraries', () => {
  ...
  test('returns an object with a libraries3D array', async () => {
    const result = await detect3DLibraries();
    expect(result).toHaveProperty('libraries3D');
    expect(Array.isArray(result.libraries3D)).toBe(true);
  });

  test('returns empty array when no 3D libraries are present', async () => {
    const { libraries3D } = await detect3DLibraries();
    expect(libraries3D).toHaveLength(0);
  });

  test('detects Three.js via script tag', async () => {
  ...
  test('detects Babylon.js via script tag', async () => {
    const script = document.createElement('script');
  ...
  test('detects A-Frame via script tag', async () => {
    const script = document.createElement('script');
  ...
  test('detects Spline via script tag', async () => {
    const script = document.createElement('script');
  ...
  test('detects model-viewer via script tag', async () => {
    const script = document.createElement('script');
  ...
  test('detects Rive via script tag', async () => {
    const script = document.createElement('script');
  ...
  test('does not duplicate libraries detected via both script and global', async () => {
